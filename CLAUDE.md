...
| 终端劫持 | **node-pty** | 封装在 `runtime/`，对 Core 透明 |
...
| `cli/*` | `event/`, `shared/`, `runtime/`, `approval/` | `transport/`, `storage/` |
...
├── cli/          # CLI 适配器 (base, claude)
├── runtime/      # 进程运行容器 (nodepty)
├── approval/     # 各 CLI 审批检测（正则 / SDK 解析）
...
6. **进程 ≠ 会话**：PTY 空闲超时只杀进程、会话转 `idle`；`closed` 只由 `/close` 或归档触发。
...
- **新增 CLI**：在 `cli/` 实现 `BaseCLIAdapter` + 在 `approval/` 加检测，`main.ts` 注册。
