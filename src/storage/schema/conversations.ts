 ...
 * 会话 scope = (platform, userId, cli)：见 docs/02-Architecture.md §5.1。
 */
...
export const conversations = pgTable(
  'conversations',
  {
    ...
    cwd: text('cwd').notNull(), // 当前会话目录；同 CLI 的未关闭会话不可原地换目录
    status: sessionStatusEnum('status').notNull().default('idle'),
    createdAt: bigint('created_at', { mode: 'number' }).notNull(),
    updatedAt: bigint('updated_at', { mode: 'number' }).notNull(),
  },
  t => [
    index('idx_conv_scope_recent').on(t.platform, t.userId, t.cli, t.updatedAt),
    // 数据库兜底：每个 scope 至多一条未关闭会话，防止并发入站建出重复会话。
    uniqueIndex('uniq_conv_open_scope')
      .on(t.platform, t.userId, t.cli)
