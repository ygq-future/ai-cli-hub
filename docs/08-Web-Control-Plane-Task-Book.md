...
| 聊天控制台 | 当前 Web 会话历史回显、流式回答、命令、文件/图片粘贴预览、消息气泡图片、工具状态、审批卡、浏览器通知与自动审批倒计时。 |
...
基于真实数据完成 loading/empty/error 等状态、键盘与触控细节、可访问性、弱网重连和多端回归；聊天历史默认只取最新 10 条，滚动到顶部通过 cursor 分页加载并保持阅读位置；图片在气泡中直接显示，其他文件按类型显示预览卡片。不得把布局、主题、强调色或国际化留到本阶段才首次实现。验收：320px、768px、1280px；中英文案完整；三种主题与五种强调色均通过真实功能回归。
