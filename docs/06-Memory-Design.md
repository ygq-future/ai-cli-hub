...
- **V1.5 自然语言记忆触发**：普通对话中用户表达“记住/记一下/记录/remember this”等意图时，Core 保存该用户消息后发 `MemorySummaryRequested`，不再交给 Claude SDK 处理，避免 SDK 自己写宿主 memory 文件。Memory 模块读取当前 conversation 最近 10 条 `messages` 表中的 user/assistant 消息，调用 LLM summary API 生成一条 conversation-derived episodic 记忆；摘要输出语言跟随当前用户 `/lang`，长度上限由 `MEMORY_SUMMARY_MAX_CHARS` 控制，并要求第三人称或中性事实陈述，避免“你/我/我们/助手”等依赖当前对话身份的人称。
- 会话关闭归档：`/close`、`/new`、`/cwd <path>` 关闭旧会话，或归档超时触发 `SessionClosed` 后，Memory 模块读取该 conversation 最近 user/assistant 消息，生成一条 `conversation-derived episodic` 摘录记忆（`conversation_id=<当前会话>`，`tag=conversation.summary:<conversationId>`）。这类记忆会回填 embedding，并进入后续 Top-K 语义召回池。
...
### 4.1 触发与节流
- 当前实现：`SessionClosed` 时生成一次会话派生摘录；少于 2 条 user/assistant 消息不生成。
- 当前摘录采用确定性规则：最近最多 12 条 user/assistant 消息，每条截断，不调用 LLM；这样行为可解释、无额外模型成本。
- 后续可增加滚动阈值 `N`（如每 20 条消息）和 LLM 摘要器，避免长会话只在关闭时归档。
