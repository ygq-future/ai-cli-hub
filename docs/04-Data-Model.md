...
```text
user_preferences: (platform, user_id) → language, default_cli, auto_approve_enabled
user_cli_cwds:    (platform, user_id, cli) → cwd
```

- 首次访问懒创建 `language='zh'`、`default_cli='claude'`、`auto_approve_enabled=false`。
...
| user_preferences | primary `(platform, user_id)` | 读取语言、默认 CLI 与自动审批开关 |
