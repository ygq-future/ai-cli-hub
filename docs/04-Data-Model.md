...
用户偏好不使用泛化 JSON KV：语言、默认 CLI 和目录都是稳定、可约束的查询字段，采用两张强类型表并以 `(platform, user_id)` 隔离。

```text
user_preferences: (platform, user_id) → language, default_cli
user_cli_cwds:    (platform, user_id, cli) → cwd
```

- 首次访问懒创建 `language='zh'`、`default_cli='claude'`。
...
| user_preferences | primary `(platform, user_id)` | 读取语言与默认 CLI |
