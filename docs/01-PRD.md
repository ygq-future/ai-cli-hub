...
* **关键区分**：`运行时回收` ≠ `会话关闭`。已启动的 CLI/adapter 空闲超时仅关闭运行时、会话转 `idle` 可随时唤醒；`/close`、`/cwd <path>` 或长期无活动才进入 `closed` 并生成摘要。
* **会话边界与 `/switch`/`/cwd`**：会话 scope = `(platform,user_id,cli)`，每个 CLI 只保留一条未关闭会话；同一用户可同时保留 Claude 与 OpenCode 上下文。`/switch <cli> [path]` 恢复目标 CLI 会话，不存在时按持久化/显式 cwd 新建，且不关闭其他 CLI。已有会话与显式 path 冲突时拒绝，避免隐式丢上下文。
...
✅ 永久 Audit Log 审计记录机制
✅ 会话边界管理（CLI 目标切换 / `/switch` / `/close` / `/cwd` / 自动归档）
