...
* **CLI Adapter（语义接缝）**：所有 CLI 工具实现统一的语义化 `CLIAdapter` 接口（含生命周期、输入输出、审批与 `listModels` / `setModel`）。它说领域语义（一轮输入 / 流式输出 / 审批请求+决定 / 模型管理 / 生命周期），与「字节还是结构化」无关。当前提供 **`ClaudeSdkAdapter`（走 `@anthropic-ai/claude-agent-sdk`）** 与 **`OpenCodeSdkAdapter`（走 `@opencode-ai/sdk`，需本机 `opencode` CLI）**。
...
* **模型管理**：`/model` 懒启动当前未关闭会话的 CLI 并以名称/复制按钮列出当前账号可用模型；`/model <model_name|model_id>` 解析规范模型、切换后续轮次，并按 `(platform,user_id,cli)` 持久化名称与 ID。没有当前 CLI 会话时不隐式创建。
