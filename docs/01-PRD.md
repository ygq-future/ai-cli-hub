...
* **会话边界与 `/new`/`/cwd`**：以 `(user_id, cli, cwd)` 三元组定位会话，保证同一边界至多一个未关闭会话。`/new` **直接关闭**旧会话（不再置 `idle`）并创建新 `idle` 会话；`/cwd <path>` 关闭当前会话并切换目标 cwd，不创建 conversation。唤醒 `idle` = 复用同一 DB 行 + 全新 Claude query，**不回放、不 resume**，跨会话连续性由记忆子系统承担。
...
> **向量分阶段**：V1 建表即预留 `embedding` 列（可 NULL），先做命令式全局记忆与环境事实注入；V1.5 启用 `pgvector` + HNSW 索引填充向量，接入语义召回。详见架构文档 §7。
...
✅ 长期记忆基础：命令式全局记忆 + 环境记忆注入（`embedding` 列预留待 V1.5 启用向量）
