...
* **CLI Adapter（语义接缝）**：所有 CLI 工具实现统一的语义化 `CLIAdapter` 接口（含生命周期、输入输出、审批与 `listModels` / `setModel`）。它说领域语义（一轮输入 / 流式输出 / 审批请求+决定 / 模型管理 / 生命周期），与「字节还是结构化」无关。当前提供 **`ClaudeSdkAdapter`（走 `@anthropic-ai/claude-agent-sdk`）** 与 **`OpenCodeSdkAdapter`（走 `@opencode-ai/sdk`，需本机 `opencode` CLI）**；后者的 `opencode serve` 在装配根按进程共享，各 adapter 使用独立 SDK session，因此 QQ 与 Telegram 可并发运行 OpenCode 会话。
...
* **关键区分**：`运行时回收` ≠ `会话关闭`。已启动的 CLI/adapter 空闲超时仅关闭运行时、会话转 `idle` 可随时唤醒；`/close` 或长期无活动才进入 `closed` 并生成摘要。
* **会话边界与 `/switch`**：会话 scope = `(platform,user_id,cli)`，每个 CLI 只保留一条未关闭会话；同一用户可同时保留 Claude 与 OpenCode 上下文。`/switch <cli> [path]` 恢复目标 CLI 会话，不存在时按持久化/显式 cwd 新建，且不关闭其他 CLI。已有会话与显式 path 冲突时拒绝，避免隐式丢上下文；更换该 CLI 的目录时先 `/close`，再以新 path 执行 `/switch`。
* **模型管理**：`/model` 懒启动当前未关闭会话的 CLI 并以名称列出当前账号可用模型；Telegram 使用原生复制按钮，QQ 将规范 ID 放入 Markdown fenced code block，使用客户端原生复制入口；`/model <model_name|model_id>` 解析规范模型、切换后续轮次，并按 `(platform,user_id,cli)` 持久化名称与 ID。没有当前 CLI 会话时不隐式创建。
...
| 表名 | 字段描述 | 用途 |
|---|---|---|
| `conversations` | `id`, `platform`, `user_id`, `cli`, `cwd`, `status`, `created_at`, `updated_at` | 记录 `(platform,user_id,cli)` scope 的会话状态与生命周期；`cwd` 是该会话的工作目录 |
...
✅ 永久 Audit Log 审计记录机制
✅ 会话边界管理（CLI 目标切换 / `/switch` / `/close` / 自动归档）
✅ 长期记忆基础：实例级命令式全局记忆 + 环境记忆注入（`embedding` 列预留待 V1.5 启用向量）
✅ 媒体/文件基础：emoji 文本归一化、sticker metadata、Telegram 可下载媒体入站保存、非图片文件懒加载 metadata/local_path、按需 DOCX 解析能力、图片 Light OCR HTTP provider 接入；PDF 交给本地 OCR 服务按需转图识别，Excel 交给外部文件处理能力（音视频转写与 Vision 后续迭代）
