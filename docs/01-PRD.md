...
* **会话边界与 `/new`/`/cwd`**：同一用户只保留最新一条可复用会话（`idle/starting/running`）；Transport 重启丢失内存目标时，普通消息和 `/status` 会回查该用户最新可复用会话并恢复 `cli/cwd`。`/new` 新建前兜底关闭该用户所有非 `closed` 历史会话；`/cwd <path>` 关闭当前会话并切换目标 cwd，不创建 conversation。唤醒 `idle` = 复用同一 DB 行 + 全新 Claude query，user message 前缀携带最近 10 条历史消息；跨会话连续性由记忆子系统承担。
