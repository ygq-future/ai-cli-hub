...
- 会话 scope：`findLatestOpen(platform,user,cli)` 复用；`/switch <cli> [path]` 在各 CLI 会话间切换、设置新会话目录且不关闭其他 CLI；按 idle 时间归档。
...
- `cli/`：`ClaudeSdkAdapter` 实现 `CLIAdapter`（§3.1），内部持 `@anthropic-ai/claude-agent-sdk` 的 `query()` 句柄；V2-R3 扩展 `OpenCodeSdkAdapter`，通过 Composition Root 共享、引用计数的 `opencode serve` 创建独立 session，并映射 SSE/permission 事件。
  - 模型管理：Claude 使用 `supportedModels()` / `setModel()`；OpenCode 列出已连接 provider 的模型并在每轮 prompt 携带所选 `{providerID,modelID}`；选择按名称或 ID 匹配并写入 `user_cli_preferences.model_id/model_name`。
...
- **R2-C 受控自更新**：通过显式命令触发 `git pull --ff-only`、`bun install`、`bun run webui:build`、`bun run db:migrate`、`bun run format:check`/`typecheck` 等检查；WebUI 构建属于部署更新步骤，普通启动/PM2 重启只读取现有产物；Claude SDK 内置平台包在 Bun 解析依赖时已被同名本地 stub override，不发生下载；成功后写入重启通知 marker 并交给 PM2/systemd 重启；新进程启动后主动通知原 chat；失败必须保留当前进程并报告原因。`/update` 与 `/restart` 仅适用于 Linux/VPS 部署，Windows 上直接拒绝执行。另提供 `/restart` 复用同一重启命令与通知 marker，仅验证重启链路，不更新代码。
